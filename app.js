    ...
    if (!state.matches || state.matches.length === 0) {
      ...
      state.matches.forEach((m, idx) => {
        ...
        li.innerHTML = `
          ...
          <button type="button" class="btn btn-danger-outline btn-sm btn-delete-match" data-match="${m.id}">🗑�?/button>
        `;
    ...
    if (players.length === 0) {
      ...
      players.forEach(p => {
        ...
        li.innerHTML = `
          ...
          <button type="button" class="btn btn-danger-outline btn-sm btn-delete-user" data-user="${p.id}">🗑�?Eliminar</button>
        `;
